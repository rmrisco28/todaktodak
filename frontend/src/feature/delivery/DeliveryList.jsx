...
export function DeliveryList() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/delivery/list?${searchParams}`)
      .then((res) => {
  ...
  function handleSearchFormSubmit(e) {
    e.preventDefault();
    navigate("/delivery/list?q=" + keyword);
  }
  ...
  function handleDeleteButtonClick() {
    axios
      ...
      .catch((err) => {
        console.log("동작 오류");
        toast("배송업체가 삭제되지 않았습니다.", { type: "warning" });
      })
      .finally(() => {
        console.log("항상 실행");
      });
  ...
  return (
    <>
      <Row>
        <Col>
          <h2 className="mb-4">배송업체 목록</h2>
          <Button variant="primary" onClick={() => navigate(`/delivery/add`)}>
            배송업체 등록
          </Button>
              ...
              <tbody>
                {deliveryList.map((delivery) => (
                  <tr key={delivery.seq}>
                    ...
                    <td>
                      <div className="d-flex gap-2">
                        <span>{delivery.callNo}</span>
                      </div>
                    </td>
                    {/*
                    <td>
                      <div className="d-flex gap-2">
                        <span>{delivery.useYn}</span>
                      </div>
                    </td>
                    */}
                    ...
                    <td>
                      <Button
                        ...
                        onClick={() =>
                          navigate(`/delivery/modify/${delivery.seq}`)
                        }
                      >
                        {/*수정*/}
                        <FaRegPenToSquare />
                    ...
                    <td>
                      <Button
                        ...
                        onClick={() =>
                          handleDeleteCategoryModalShow(delivery.seq)
                        }
                      >
                        {/*삭제*/}
                        <FaXmark />
      ...
      <Row className={"my-3"}>
        ...
        <Modal show={modalShow} onHide={() => setModalShow(false)}>
          <Modal.Header closeButton>
            <Modal.Title>배송업체 삭제 확인</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {deleteTarget} 번 배송업체를 삭제하시겠습니까?
          </Modal.Body>
          <Modal.Footer>
